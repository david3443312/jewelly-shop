...
if(isset($_POST['delete'])){
    ...
    $delete_request = $conn->prepare("DELETE FROM `couple_designs` WHERE id = ?");
    $delete_request->execute([$delete_id]);
    
    header('location: couple_designs.php');
}
...
if(isset($_POST['update_status'])){
    ...
    $update_status = $conn->prepare("UPDATE `couple_designs` SET status = ? WHERE id = ?");
...
<head>
    ...
    <title>Couple Design Requests</title>
...
<body>
    ...
    <section class="show-products">
        <h1 class="heading cs_heading" >Couple Design Requests</h1>
        <div class="box-container">
            <?php
            $select_requests = $conn->prepare("SELECT * FROM `couple_designs` ORDER BY created_at DESC");
