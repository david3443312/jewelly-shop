...
if(isset($_GET['id'])){
    ...
    $select = $conn->prepare("SELECT * FROM `couple_designs` WHERE id = ?");
    $select->execute([$id]);
...
if(isset($_POST['update'])){
    ...
    $update = $conn->prepare("UPDATE `couple_designs` SET name=?, email=?, phone=?, design_type=?, description=?, budget=?, deadline=?, status=? WHERE id=?");
...
<head>
    ...
    <title>Edit Couple Design</title>
...
<body>
    ...
    <section class="form-container">
        <form action="" method="POST">
            <h3>Edit Couple Design Request</h3>
            ...
            <a href="couple_designs.php" class="option-btn">Back</a>
        </form>
    </section>
</body>
</html>
