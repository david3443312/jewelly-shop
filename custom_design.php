...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Tạo ID ngẫu nhiên cho custom design
        $id = substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 20);
        
        $stmt = $conn->prepare("INSERT INTO custom_designs (id, user_id, name, email, phone, design_type, description, budget, deadline, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
        $stmt->execute([$id, $user_id, $name, $email, $phone, $design_type, $description, $budget, $deadline]);
        
        $success_message = "Yêu cầu thiết kế của bạn đã được gửi thành công! Chúng tôi sẽ liên hệ với bạn trong thời gian sớm nhất.";
...
<head>
    ...
    <title>Đặt Trang Sức Thiết Kế Riêng - Jewelry Shop</title>
...
<body>
    ...
    <div class="container">
        <div class="custom-design-form">
            <h1>Đặt Trang Sức Thiết Kế Riêng</h1>
