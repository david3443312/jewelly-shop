...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // Tạo ID ngẫu nhiên cho group design
        $id = substr(str_shuffle('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'), 0, 20);
        
        $stmt = $conn->prepare("INSERT INTO group_designs (id, user_id, name, email, phone, design_type, description, budget, deadline, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
        $stmt->execute([$id, $user_id, $name, $email, $phone, $design_type, $description, $budget, $deadline]);
        
        $success_message = "Yêu cầu thiết kế trang sức theo nhóm của bạn đã được gửi thành công! Chúng tôi sẽ liên hệ với bạn trong thời gian sớm nhất.";
...
<head>
    ...
    <title>Đặt Trang Sức Theo Nhóm - Jewelry Shop</title>
    ...
    <div class="container">
        <div class="custom-design-form">
            ...
            <form method="POST" action="" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="design_type">Loại trang sức *</label>
                    <select id="design_type" name="design_type" required>
                        <option value="">Chọn loại trang sức</option>
                        <option value="group_ring">Nhẫn</option>
                        <option value="group_bracelet">Vòng tay</option>
                        <option value="group_necklace">Vòng cổ</option>
                        <option value="group_earring">Khuyên tai</option>
                        <option value="other">Khác</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="group_size">Số lượng cần thiết kế *</label>
                    <input type="number" id="group_size" name="group_size" min="3" required placeholder="Nhập số lượng (tối thiểu 3)">
                </div>
